...
if page == "📊 Dashboard":
    ...
    st.markdown(
        '<div class="main-header">Fitness AI Dashboard</div>',
        unsafe_allow_html=True
    )
...
# =============================================================================
    ...
    with st.form("add_user_form"):
        ...
        with col2:
            new_health = st.selectbox(
                "Health Condition",
                ["Healthy", "Asthma", "Heart Condition", "Diabetes"]
            )
        ...
        if submit_user:
            if not new_username or not new_password:
                ...
                try:
                    ...
                    # Get city_id if city name provided
                    city_id = None
                    if new_city:
                        city_result = load_dataframe(
    ...
    if st.button("Delete User", type="secondary"):
        try:
            ...
            admin_count = load_dataframe(
                "SELECT COUNT(*) as count FROM users WHERE is_admin = 1"
            ).iloc[0]["count"]

            is_admin = load_dataframe(
                "SELECT is_admin FROM users WHERE id = ?", (user_id,)
            ).iloc[0]["is_admin"] if user_id else False
